...
function Phone() {
  ...
  const topic="Điện thoại";
  useEffect(()=>{
    const fetchApi =async ()=>{
      try{
        const resultProduct = await getProducsByCategory("dien-thoai");
        const resultBrand = await getBrandsByCategory("dien-thoai");
            ...
            <Col span={18}>
              <div className="product__category">
                ...
                {brands.map((brand,index)=>(
                  <>
                    <li key={index}>
                      <a href={"/dien-thoai?filter=brand:" +brand.name} >{brand.name}</a>
                    </li>
                  </>
                ))}
                <li>
